...
export default class Pedido {
  ...
  getResumen() {
    return `${this._fecha.getFecha()} ${this._hora.getFormato12()} - ${this.getNumeroElementos()} elementos con ${this.getNumeroProductos()} productos - total: 
    ${new Precio(this.getCostoTotal()).getPrecio()} `;
  }
  ...
  getCostoTotal() {
    ...
    this._elementosPedidos.forEach(elemento => {
      costoTotal =
        costoTotal + elemento.getCantidad() * elemento.getPrecio();
    });
